<div class="row">
    ...
    <div id="basketSummary" class="col-12 col-md-6 summary[{if $oViewConf->getShowVouchers() && $oViewConf->getActiveClassName() != 'basket'}] offset-md-6[{/if}][{if $oViewConf->getActiveClassName() == 'order'}] orderSummary[{/if}]">
        ...
        <table class="table table-bordered table-striped">
            ...
            [{if $oViewConf->isFunctionalityEnabled('blShowVATForDelivery') || $oxcmp_user->inGroup('oxiddealer') }]
            <tr>
            ...
            <tr>
                <th class="text-right">[{ oxmultilang ident="SHIPPING_COST" suffix="COLON" }]
                    [{if $oxcmp_basket->hasSurcharge()}]
                <br><small>([{oxmultilang ident="SURCHARGE_APPLIED"}])</small>
                    [{elseif $oxcmp_user->inGroup('oxiddealer') && $deliveryCost->getBruttoPrice() > 0 && $oxcmp_user->getIslandSurcharge() == 0}]
                <br><small>([{oxmultilang ident="SHIPPING_NOTE"}])</small>
                    [{/if}]
                </th>
