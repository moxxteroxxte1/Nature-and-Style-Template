        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse navbar-main-collapse" id="navbarSupportedContent">
                ...
                <ul id="navigation" class="navbar-nav nav">
                    ...
                    [{assign var="aSubCats" value=$oTopCont->getSubCats()}]
                    [{if count($aSubCats) > 0}]
                    <li class="nav-item[{if $oContent->oxcontents__oxloadid->value === $oTopCont->oxcontents__oxloadid->value}] active[{/if}] dropdown">
                        <a class="nav-link" href="[{$oTopCont->getLink()}]" class="dropdown-toggle" data-toggle="dropdown">
                            [{$oTopCont->oxcontents__oxtitle->value}] <i class="fa fa-angle-down"></i>
                        </a>
                        <ul class="dropdown-menu">
                            [{foreach from=$aSubCats item='aCont' name="MoreSubCms"}]
                                [{foreach from=$aCont item='cont'}]
                                    [{if $cont->isActive()}]
                                        <li class="dropdown-item[{if $oViewConf->getContentId() == $cont->getId()}] active[{/if}]">
                                            <a class="dropdown-link[{if $oViewConf->getContentId() == $cont->getId()}] current[{/if}]" href="[{$cont->getLink()}]">[{$cont->oxcontents__oxtitle->value}]</a>
                                        </li>
                                    [{/if}]
                                [{/foreach}]
                            [{/foreach}]
